...
# на реальном примере. Необходимо запрашивать у пользователя данные и заполнять список. Класс-исключение должен контролировать типы данных 
# элементов списка.
...
result = []
while True:
	element = input('Введите число для добавления в список или stop для выхода: ')
	...
	try:
		if not element.replace('.','',1).isdigit():
			raise Check_numbers('txt')
		result.append(float(element))
	except Check_numbers as error:
